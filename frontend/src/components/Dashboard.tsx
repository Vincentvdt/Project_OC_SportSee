...
import { UserAPIResponse, UserData, UserInfo } from "../interfaces";
...
import { getUserActivity, getUserAverageSessions, getUserInfo, getUserPerformance } from "../services/userService.tsx";
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const [userInfoData, activityData, performanceData, averageSessionsData] = await Promise.all([
                    getUserInfo(userId),
    ...
    return (
        <DashboardContainer>
            <DashboardHeader>
                <h1>Bonjour <span
                    onClick={() => userId === 12 ? setUserId(18) : setUserId(12)}>{isLoading ?
                    <LoaderIcon/> : userData.user.userInfos.firstName}
                    </span> {mock ? <small>mock data</small> : null}
                </h1>
            ...
            <MainLayout>
                ...
                <MacrosLayout>
                    {(Object.keys(userData.user.keyData).map((key) => {
                        ...
                        return <MacroCard key={key} name={key as keyof typeof userData.user.keyData} value={value}/>;
                    }))}
                </MacrosLayout>
            </MainLayout>
        </DashboardContainer>
